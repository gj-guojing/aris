...
void test_householder(){
		...
		, Size a_t, Size u_t, Size tau_t, Size q_t, Size r_t, Size x_t, Size b_t, Size ss_t, Size pinv_t)->void
	{
		...
		if (!(s_is_equal(m, n, r1_, U, error) && s_is_equal(std::min({ m - 1, m, n }), r2_, tau, error)))
			std::cout << "\"s_householder_ut\" failed" << std::endl;

		s_mc(m, n, A, n, i1_, a_t);
		s_householder_ut(m, n, i1_, a_t, r1_, u_t, r2_, tau_t);
		if (!(s_is_equal(m, n, r1_, u_t, U, n, error) && s_is_equal(std::min({ m - 1, m, n }), 1, r2_, tau_t, tau, 1, error)))
			std::cout << "\"s_householder_ut\" failed" << std::endl;
		...
		s_householder_u2q(m, n, U, r1_);
		if (!s_is_equal(m, m, r1_, Q, error)) 
			std::cout << "\"s_householder_u2q\" failed" << std::endl;

		s_mc(m, n, U, n, i1_, u_t);
		s_householder_u2q(m, n, i1_, u_t, r1_, q_t);
		if (!s_is_equal(m, m, r1_, q_t, Q, m, error))
			std::cout << "\"s_householder_u2q\" failed" << std::endl;

		s_householder_u2qmn(m, n, U, r1_);
		if (!s_is_equal(m, std::min(m, n), r1_, n, Q, m, error))std::cout << "\"s_householder_u2q\" failed" << std::endl;

		s_mc(m, n, U, n, i1_, u_t);
		s_householder_u2qmn(m, n, i1_, u_t, r1_, q_t);
		if (!s_is_equal(m, std::min(m, n), r1_, q_t, Q, m, error))std::cout << "\"s_householder_u2q\" failed" << std::endl;

		s_householder_u2r(m, n, U, r1_);
		if (!s_is_equal(m, n, r1_, R, error))std::cout << "\"s_householder_u2r\" failed" << std::endl;

		s_mc(m, n, U, n, i1_, u_t);
		s_householder_u2r(m, n, i1_, u_t, r1_, r_t);
		if (!s_is_equal(m, n, r1_, r_t, R, n, error))std::cout << "\"s_householder_u2r\" failed" << std::endl;

		s_householder_u2qr(m, n, U, r1_, r2_);
		if (!(s_is_equal(m, m, r1_, Q, error) && s_is_equal(m, n, r2_, R, error)))std::cout << "\"s_householder_u2qr\" failed" << std::endl;

		s_mc(m, n, U, n, i1_, u_t);
		s_householder_u2qr(m, n, i1_, u_t, r1_, q_t, r2_, r_t);
		if (!(s_is_equal(m, m, r1_, q_t, Q, m, error) && s_is_equal(m, n, r2_, r_t, R, n, error)))std::cout << "\"s_householder_u2qr\" failed" << std::endl;
	...
	// test 6*8 mat, rank = 3	
		...
		const double tau[m]{
			-1.21659046950618, - 1.17718507711822, - 1.37293669646709,   -2,   -2 };
		...
		const double tau_p[n]{
			-1.21659046950618, - 1.00944244215200, - 1.50159172513588,   -2,   -2
		};
...
void test_svd() {
	auto test_svd_mat = [](aris::Size m, aris::Size n, const double* A_old, auto a_t_old) {
		...
		{
			std::fill_n(U_data_.data(), U_data_.size(), -9999.0);
			std::fill_n(V_data_.data(), V_data_.size(), -9999.0);
			std::fill_n(S_data_.data(), S_data_.size(), -9999.0);
			...
			if (!s_is_equal(m, n, result2_data_.data(), n, A, a_t, 1e-10)) {
				std::cout << "svd error 1:multiply error!" << std::endl;
			...
			if (!s_is_equal(m, m, result3_data_.data(), result4_data_.data(), 1e-10)) {
				std::cout << "svd error 2:U not orth!" << std::endl;
			...
			if (!s_is_equal(n, n, result5_data_.data(), result6_data_.data(), 1e-10)) {
				std::cout << "svd error 3:V not orth!" << std::endl;
				dsp(n, n, result5_data_.data());
				std::exit(0);
			}

			if (std::count(U_data_.begin(), U_data_.end(), -9999.0) != (7 * m + 12)) {
				std::cout << "svd error 4:U mem error" << std::endl;
				aris::dynamic::dsp(m + 3, m + 4, U_data_.data());
				std::exit(0);
			}
			if (std::count(V_data_.begin(), V_data_.end(), -9999.0) != (15 * n + 56)) {
				std::cout << "svd error 5:V mem error" << std::endl;
				aris::dynamic::dsp(n + 7, n + 8, V_data_.data());
				std::exit(0);
			}
			if (std::count(S_data_.begin(), S_data_.end(), -9999.0) != (5 * n + 6 * m + 30)) {
				std::cout << "svd error 6:S mem error" << std::endl;
				aris::dynamic::dsp(m + 5, n + 6, S_data_.data());
				std::exit(0);
			}

			for (aris::Size i = 1; i < std::min(m, n); ++i) {
				if ((S[at(i, i, s_t)] - S[at(i - 1, i - 1, s_t)]) > 0) {
					std::cout << "svd error 7:S not decend" << std::endl;
					aris::dynamic::dsp(m, n, S, s_t);
					std::exit(0);
				}
			}
		};
	...
	test_svd_mat(15, 15, A, 15);
	test_svd_mat(20, 15, A, 15);
	test_svd_mat(25, 15, A, 15);
	test_svd_mat(15, 16, A, 15);
...
void test_qp() {
	...
	{
		const aris::Size nG = 3, nCE = 1, nCI = 1;

		double G[nG* nG]{ 1.86731018525676, 0.676506748114136,  1.83244622243266,
		0.676506748114136, 0.438313983411573, 0.927636560784582,
		1.83244622243266, 0.927636560784582,  2.42107072898398 };

		double g[nG]{ 0.792207329559554, 0.959492426392903, 0.655740699156587 };

		double CE[nG]{ 0.0357116785741896, 0.933993247757551, 0.757740130578333 };
		double ce[nCE]{ 0.392227019534168 };

		double CI[nG]{ 0.849129305868777, 0.678735154857773, 0.743132468124916 };
		double ci[nCI]{ 0.655477890177557 };
...

	{
		...
		double G[nG * nG]{ 1.59528178919379,1.28954033212097,1.17487853301547,1.72516198998871,1.41353416207074,1.98661099180372,
		  1.28954033212097,2.54405279114091,1.81938652472554,1.60760860760143,2.21446508990191,1.70108024664331,
		  1.17487853301547,1.81938652472554,1.58859287793054,1.54644176387638,1.72490830785575,1.50271126526609,
		  1.72516198998871,1.60760860760143,1.54644176387638,2.20221635392974,1.51141261229934,2.04076557030327,
		  1.41353416207074,2.21446508990191,1.72490830785575,1.51141261229934,2.34110159478375,1.99074329038053,
		  1.98661099180372,1.70108024664331,1.50271126526609,2.04076557030327,1.99074329038053,2.60550339386604 };

		double g[nG]{ 0.225921780972399,0.170708047147859,0.227664297816554,0.435698684103899,0.311102286650413,0.923379642103244 };

		double CE[nCE*nG]{ 0.430207391329584,0.979748378356085,0.258064695912067,0.262211747780845,0.22174673401724,0.318778301925882,
		 0.184816320124136,0.438869973126103,0.408719846112552,0.602843089382083,0.117417650855806,0.424166759713807,
		 0.904880968679893,0.111119223440599,0.594896074008614,0.711215780433683,0.296675873218327,0.507858284661118 };
		double ce[nCE]{ 0.085515797090044,0.262482234698333,0.801014622769739 };

		double CI[nCI * nG]{ 0.0292202775621463,0.578525061023439,0.546805718738968,0.62406008817369,0.987982003161633,0.796183873585212,
		 0.928854139478045,0.237283579771521,0.521135830804001,0.679135540865748,0.0377388662395521,0.0987122786555743,
		 0.730330862855453,0.458848828179931,0.231594386708524,0.395515215668593,0.885168008202475,0.261871183870716,
		 0.488608973803579,0.963088539286913,0.488897743920167,0.367436648544477,0.913286827639239,0.335356839962797 };
		double ci[nCI]{ 0.679727951377338,0.13655313735537,0.72122749858174,0.106761861607241 };

		double x[nG]{ -0.3668876591373093,   0.4873201142058127, - 3.4943049188263844,   3.0580877261171286,   2.3516802032933337, - 2.5933895021872431 };

		//test_qp(nG, nCE, nCI, G, g, CE, ce, CI, ci, x);
	}

	{
		const aris::Size nG = 6, nCE = 1, nCI = 7;
		...
		double CE[nCE * nG]{ 0.430207391329584,0.979748378356085,0.258064695912067,0.262211747780845,0.22174673401724,0.318778301925882,};
		double ce[nCE]{ 0.085515797090044 };

		double CI[nCI * nG]{ 0.8147,0.5469,0.8003,0.0357,0.6555,0.8235,
							0.9058,0.9575,0.1419,0.8491,0.1712,0.6948,
							0.1270,0.9649,0.4218,0.9340,0.7060,0.3171,
							0.9134,0.1576,0.9157,0.6787,0.0318,0.9502,
							0.6324,0.9706,0.7922,0.7577,0.2769,0.0344,
							0.0975,0.9572,0.9595,0.7431,0.0462,0.4387,
							0.2785,0.4854,0.6557,0.3922,0.0971,0.3816 };
		double ci[nCI]{ -10.0119,0.3371,0.1622,-6.7943,0.3112,0.5285,-12.1656 };
